...
const eventAudioCache = new Map();
const EVENT_AUDIO_FILES = {
  reach: "/audio/reach.mp3",
...
function getEventAudio(type) {
  if (!EVENT_AUDIO_FILES[type]) return null;
  if (!eventAudioCache.has(type)) {
    const audio = new Audio(EVENT_AUDIO_FILES[type]);
    audio.preload = "auto";
    audio.playsInline = true;
    eventAudioCache.set(type, audio);
  }
  return eventAudioCache.get(type);
}

function primeEventAudio() {
  if (studentAudioPrimed) return;
  studentAudioPrimed = true;
  Object.keys(EVENT_AUDIO_FILES).forEach(type => {
    const audio = getEventAudio(type);
    if (!audio) return;
    audio.load();
...
function playEventVoice(type, { volume = 1 } = {}) {
  ...
  for (const [otherType, otherAudio] of eventAudioCache.entries()) {
    if (otherType !== type) {
      otherAudio.pause();
...
function renderHost(data) {
  ...
  if (stats.reach > lastHostReachCount && hostAudioUnlocked && stats.bingo === lastHostBingoCount) {
    playEventVoice("reach", { volume: 0.95 });
    showToast("リーチ！", 1800);
  }
  lastHostReachCount = stats.reach;

  if (stats.bingo > lastHostBingoCount && hostAudioUnlocked) {
